 ...
 *   4. bump taste_version + last_updated, save (CAS on updated_at), drop the
 *      DNA read-cache.
 *
 * NOT idempotent for strand B: a second run starts from the first run's
 * values (only confidence is reset), so run it once. A and C are reset to a
 * fixed baseline and are safe to re-run.
 */
