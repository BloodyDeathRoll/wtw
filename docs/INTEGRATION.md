...
- **`getWatchProviders()` in `src/lib/tmdb.ts`** — narrows TMDB's ~200-region response to `WATCH_REGIONS` (default `US,GB,IL`) and drops rent/buy tiers, because "Watch on X" has to mean *included with a subscription*.
