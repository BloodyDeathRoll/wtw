...
- **Horror/anime bias + user rules not applied — analysed 2026-09-06; #1 fixed on `fix/enrichment-mistral-budget`, the rest open.** Findings (measured live): the Mistral workspace has answered 429 with a per-minute limit of 0 since 2026-09-04 (last successful enrichment 00:31 UTC that day; 0 of 900 enriched on 09-05 and 09-06 with 124 pending — the Dream summary carried `enrich_failing` both nights and nobody saw it). Every Mistral call in the app fails silently on that: session-end directive extraction (rule never written, chat still says "got it"), rerank (composite order), fingerprint embedding (popularity pool only). The "no horror" instructions of 09-04 17:31 UTC hit exactly that. The "no anime" instructions predate #58 (08-29) and were never re-extracted. Separately: the catalog is 20% horror (movies) / 19% anime (TV) because the grow sweep gives every genre an equal slice (top-150-by-votes is 5% / 9%), the narrative pool inherits it (25–44% horror movie pools, 19–36% anime TV pools, measured on three users incl. a warm/hopeful one); the fingerprint has no genre/format/language dimension so 27 disliked anime can't generalise; a blank fingerprint embeds as `Tone: dark, warm` (jsonb key order + stable sort on equal weights); the enrichment tags 58% of the catalog `dark`; card ratings deliberately don't bump `taste_version`, so the batch stays until session end.
  ...
  | # | Fix | Owner |
  |---|---|---|
  | 1 | ✅ `fix/enrichment-mistral-budget` (2026-09-06): batch path reads `MISTRAL_BATCH_API_KEY` (fallback `MISTRAL_API_KEY`), `maxRetries: 0`, every call counted (`src/lib/mistral-batch.ts`); `runNightlyEnrichment` stops on the first 429 (**superseded 2026-09-10, see 1c**) and on a per-run call cap; `grow-catalog` takes `MISTRAL_CALL_BUDGET` (default 600), reports `mistral_calls` / `mistral_rate_limited` / `mistral_budget_exhausted` and sets `ok:false` on a rate-limited night. Dream: `enrich_max` 900 → 300, `mistral_call_budget: 600`, run.sh hoists both flags into the digest front block. **Root cause found 2026-09-07:** the key is fine — Mistral moved `mistral-small`/`medium`/`magistral`/`devstral` off the free tier (0 req/min), while `ministral-8b-latest` / `open-mistral-nemo` (188 req/min), `ministral-3b-latest` (750) and `mistral-embed` still work. `MODELS.structured` and `MODELS.enrichment` now point at `ministral-8b-latest` (`fix/mistral-free-tier-model`); one live title enriched through `runNightlyEnrichment` on the new model (2 calls, no 429). Embeddings unchanged, so no re-embedding. | A2 (+ Dream manifest) |
  | 1b | ✅ `fix/enrich-queue-select-error` (2026-09-09): the pending-titles / pending-crew selects in `runNightlyEnrichment` discarded their `error`, so one transient Supabase failure (`fetch failed`) read as an empty backlog and `grow-catalog` exited "done" with budget and time unspent (Dream 2026-09-08: 73 of 300, no 429, no failures — the only silent-exit path in that loop; that night's instance is not provable from logs). Now: 3 attempts 2 s apart, each logged; the titles select then throws (nothing spent yet), the crew select returns the partial report with `crew_queue_failed` (Phase 1 counts are real). `grow-catalog` catches the throw as a stall (retries once after 5 s) and reports `enrich_queue_failures` (and `crew_queue_failures`) in the summary; `/api/admin/seed` reports it as `enrich_error` next to the seed counts. | A2 |
  | 1c | ✅ `enrich-cooldown-429` (2026-09-10, PR #63): stop-on-the-FIRST-429 (row 1) was right for a spent workspace and wrong for a paced one. Dream 2026-09-10: 236 calls landed, then ONE 429 ended the phase at 96 of 300 with 1,417 pending and ~2h of the job's 180m cap unspent. A 429 now cools down (the response's `retry-after`, else 60s, capped) and retries the SAME item; a success resets the streak; **three 429s in a row, each through a cooldown, is the wall** and sets `rate_limited`. That is 3 wasted calls in the 09-04 shape against 15 before. The per-run call budget is re-checked before every attempt, so a retry can never overspend it; titles and crew lineage both go through one `attempt()` helper. `mistralRetryAfterMs()` in `src/lib/mistral-batch.ts` reads the header (seconds or HTTP date; empty reads as "did not say"). Both serverless callers (`/api/cron/enrich`, `/api/admin/seed`) pass `SERVERLESS_COOLDOWN_CAP_MS` (15s) — the 5-minute default would otherwise let one `retry-after` get the function killed mid-sleep, returning no report at all; `/api/admin/seed` declares no `maxDuration` and seeds from TMDB first, so it has the least room. | A2 |
  ...
  | 9 | ✅ `feat/refresh-batch-on-ratings` (2026-09-20): every 5th card rating promotes the already-parked precompute batch to the live cache under a bumped `taste_version` (`src/modules/dna/refresh-batch.ts`). **Agreed cache cost: none.** A bump was expensive only because nothing was cached under the new version — but `precomputeNextBatch` already builds and parks a fresh batch after every rating, so the refresh is the same promotion `session/end` does (`adoptPendingBatch`), done earlier. One DNA read, one conditional write, one Redis set per 5 ratings, plus an embedding row update that only calls Mistral when the strand text actually moved. "Find more" afterwards usually takes the fast-path no-op instead of regenerating, so this is a net saving. Safety: the version is saved **only after** the batch is cached under it (a bumped version with no cache drops the feed to a cold regen, or to mocks), and the save is a compare-and-set on `updated_at` — this runs in `after()`, concurrently with the next click, which is itself a read-modify-write of the same column. Nothing is yanked from under the user: the client appends pages and dedups by id. | A3 with A2 |
...
- **Scoring + fingerprint data — fixed 2026-08-28 (measured on the reporter, 250 ratings).** Three layers, all live:
  ...
  3. *Chat signals duplicated and contradicted* — the session merge deduped on `type:id:source` and chat sources are `session_N`, so The Fifth Element / Strange Days / The Matrix landed 7–15× each (loved AND disliked) and inflated strand A on every pass. Fixed: one signal per `type:tmdb_id` across all sources, first wins. `scripts/rebuild-strands.mts` (applied, 6 users) collapsed the duplicates (56 on the reporter's account) and replayed the unique signals through the fixed rules to rebuild strands A, B, C. Legacy `mixed` reactions (dropped in 0013, 16 still stored across two users) are skipped by every rule — indexing a delta table with them produced NaN.
  Result on the reporter: embedding text went from `Pacing: moderate. Tone: hopeful, comedic. Moral ambiguity: medium…` to `Pacing: high octane. Tone: dark, hopeful. Moral ambiguity: low. Emotional demand: medium high. Protagonist: flawed self aware. Strong ensemble.`; strand A top = Tarantino / Lucas / Ford / Spielberg / Nolan (was the Matrix cast at n=20); top picks Furiosa / Star Trek / F1 / Person of Interest, bottom Harry Potter / Shrek / Amélie / The Notebook. Still open, not a defect: action anime is dense in the top 50 (the narrative pool draws them for "high octane, dark") — tune only if the reporter says it's wrong.
...
- ⚠️ **Attribution is mandatory and currently MISSING.** TMDB sources this from JustWatch and requires the attribution to be visible wherever the data is shown. The "via JustWatch" eyebrow used to sit inside the pill; it was **removed from `WherePill` on 2026-08-29 at the user's request** (the card shows the service name and icon only). It must reappear somewhere — a footer or an about/legal line — before this ships publicly. Do not treat its absence as settled.
- **One brand per title, with an icon (2026-08-28).** TMDB lists every storefront variant ("Netflix Standard with Ads", "HBO Max Amazon Channel", "Paramount+ Roku Premium Channel") and ranks live-TV bundles first — on the 300 checked titles fuboTV was the #1 entry more often than Netflix. `src/lib/watch-providers.ts` collapses raw names onto a canonical brand (`key` + `label`) and `pickWatchProvider` chooses by our preference order (subscription brands first, "… Amazon Channel" add-ons are the channel not Prime, live-TV bundles last, unknown names kept with a generic icon). `primaryWatchProvider` in `tmdb.ts` delegates to it, so `Recommendation.where` is always the brand label. The UI renders `WherePill` (icon from `src/app/components/ProviderIcon.tsx` — hand-drawn brand-coloured monograms, not the trademarked logos) directly under the year/runtime/rating meta on the compact card, full card, watchlist card and the Why detail. Since 2026-08-29 the pill is the icon + brand name only — no "Watch on" prefix, no JustWatch eyebrow (see the attribution warning above). **Checked on demand per batch (decided 2026-08-28):** `ensureWatchProviders` (`src/modules/engine/enrichment/ensure-watch-providers.ts`) checks every title in a generated batch — inline in the precompute path, in `after()` on a direct regeneration — and the GET route checks the ≤6 unchecked titles on each page it serves before rendering, so a card shows the line whenever TMDB lists a provider. Same write shape and 45-day recheck as the nightly job (which stays, for the long tail). Throttled to 4 concurrent / 350 ms gap under TMDB's 40 req / 10 s.
